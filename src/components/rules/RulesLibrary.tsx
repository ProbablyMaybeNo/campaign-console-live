...
import { usePdfIndexer, useLlamaParseIndexer } from "@/hooks/useRulesIndexer";
...
export function RulesLibrary({ open, onOpenChange, campaignId, isGM }: RulesLibraryProps) {
  ...
  const handleIndex = (source: RulesSource) => {
    if (source.type === "pdf" && source.storage_path) {
      const indexPromise = source.type_source === "llamaparse"
        ? llamaParseIndexer.indexWithLlamaParse(source.id, campaignId, source.storage_path)
        : pdfIndexer.indexPdf(source.id, campaignId, source.storage_path);
      indexPromise
        .then((result) => {
          if (result.success) {
            toast.success(`Indexed ${result.stats?.pages || result.stats?.pagesExtracted || 0} pages`);
          } else {
                          ...
                          {isGM && (
                            <div className="flex items-center gap-1 shrink-0">
                              ...
                              <TerminalButton
                                ...
                                disabled={source.index_status === "indexing" || indexSource.isPending || isLocalIndexing}
