...
function findTableHeaderContext(lines: string[], tableStartLine: number): string {
  const slice = lines
    .slice(Math.max(0, tableStartLine - 6), tableStartLine)
    .map((line) => line.trim())
    .filter(Boolean);
  const heading = findNearestHeading(slice);
  const context = heading ? [heading, ...slice.filter((line) => line !== heading)] : slice;
  return context.join(" | ");
}
...
function cleanPdfText(text: string): string {
  ...
  const dehyphenated = mergeHyphenatedLineBreaks(normalized.split("\n")).join("\n");
  const merged = mergeSoftLineBreaks(dehyphenated);
...
function mergeSoftLineBreaks(text: string): string {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const nextIsList = isListLine(nextLine);
    const isTableLine = isTableLikeLine(line);
    const nextIsTable = isTableLikeLine(nextLine);
    const endsSentence = /[.!?":]$/.test(line);
    const lineIsHeading = isHeadingLine(line);
    const nextIsHeading = isHeadingLine(nextLine);

    if (!endsSentence && nextLine && !isList && !nextIsList && !isTableLine && !nextIsTable && !lineIsHeading && !nextIsHeading && /^[a-z]/.test(nextLine)) {
...
function findNearestHeading(lines: string[]): string | null {
...
function mergeHyphenatedLineBreaks(lines: string[]): string[] {
  const result: string[] = [];
  for (let i = 0; i < lines.length; i++) {
    const line = lines[i] ?? "";
    ...
    const shouldMerge =
      ...
      !isTableLikeLine(trimmed) &&
      !isTableLikeLine(nextTrimmed);

    if (shouldMerge) {
      result.push(`${trimmed.slice(0, -1)}${nextTrimmed}`);
      i += 1;
      continue;
    }
...
export function removeHeadersFooters(pages: PDFPage[]): PDFPage[] {
  ...
  headerCounts.forEach((count, line) => {
    if (count >= threshold && line.length >= 4 && line.length < 80) {
