...
const INTERNAL_DOMAIN = "campaignconsole.internal";

/** If input has no '@', treat it as a username and build a synthetic email */
function resolveEmail(input: string): string {
  const trimmed = input.trim();
  if (trimmed.includes("@")) return trimmed;
  return `${trimmed.toLowerCase().replace(/[^a-z0-9_-]/g, "")}@${INTERNAL_DOMAIN}`;
}
...
export default function Auth() {
  ...
  const [email, setEmail] = useState(""); // signup optional email / forgot email
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (mode === "forgot") {
        ...
        const resolvedEmail = resolveEmail(identifier);
        const { error } = await signIn(resolvedEmail, password);
        if (error) throw error;
        toast.success("Access granted");
      } else {
        // Signup: username required, email optional
        ...
        const signupEmail = email.trim()
          ? email.trim()
          : `${trimmedUsername.toLowerCase()}@${INTERNAL_DOMAIN}`;

        const { error } = await signUp(signupEmail, password, trimmedUsername);
        if (error) throw error;

        const { error: signInError } = await signIn(signupEmail, password);
                ...
                {mode === "signup" && (
                  <>
                    ...
                    <div className="space-y-1.5">
                      ...
                      />
                      <p className="text-[10px] text-muted-foreground">
                        Without an email you won't be able to reset your password
                      </p>
                    </div>
                  </>
                )}
                ...
                {mode === "forgot" && (
                  <div className="space-y-1.5">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Only works if you signed up with an email address.
                    </p>
                  </div>
                )}
